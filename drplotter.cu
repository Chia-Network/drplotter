...
void gpu_find_tx_matches(uint16_t table, uint32_t batch_id, uint32_t start_kbc_L, uint32_t end_kbc_R,
	...
	for (int i=threadIdx.x;i < total_matches;i+=blockDim.x) {
		...
		if (table == 2) {
			// this task can be left to the CPU to deal with the batch buckets.
			// const uint32_t lxs[2] = { L_entry.meta[0], L_entry.meta[2], R_Entry.meta[0], R_Entry.meta[2] };
			// TODO: sort into gpu buffer of kbc's buckets and export and write to disk.
		}
		if ((table == 3) || (table == 4) || (table == 5) || (table == 6)) {
			// get a reference table, so we can prune later
			// these could be compressed into local_kbc (25-6=19), idxL 9 idxR 9 = total 37 bits -> 48 bits = 6 bytes.
			// but we need to round up to 4 bytes anyway, so what the heck.
			BackRef ref = {};
			ref.prev_batch_kbc_id = (batch_id << 26) + global_kbc_L_bucket_id;
			ref.prev_idxL = match.idxL;
			ref.prev_idxR = match.idxR;
			*ref_data = (BackRef *) device_buffer_refdata;
			//ref_data[pair_address] = ref;
			// in gpu_merge_block_buckets_into_kbc_buckets we use ith entry, we then STORE it
			//  i.e. struct Tx_Bucketed_Meta2_with_ref {
			//     uint32_t meta[2];
			//     uint32_t y;
			//     uint32_t block_backref; // either global or include block_id, but not necessary at that point right?
			//              = (block_id << (32 - 6)) + block_pos;
		    //};
			//  now cpu doesn't need to do any work, just dump to file.
			// and our backrefs are:
			// struct BackRef {
			...
			// add to kbc bucket. Or leave for CPU to sort and save?
		}
		...
		uint32_t KBC_START, uint32_t KBC_END)
{
	...
	for (uint32_t x_group = index; x_group <= end_n; x_group += stride) {
		...
		// k25 version ug complicated since it uses stream and bits can carry into next 512 bits.
		// 1st = [x0:first 25 bits]
		//uint32_t x25_0 = (x0 & 0b11111111111111111111111110000000) >> (32-25);
		//2nd = [x0:bits 26,27,28,29,30,31,32][x1:bits 1-18, ]
		//uint32_t x25_1 = (x0 & 0b00000000000000000000000001111111) << (32-25) +     // 7
		//		         (x1 & 0b11111111111111111100000000000000) >> (25-(32-25)); // 18
	}
}
		...
		BUCKETED_ENTRY_BLOCKPOSREF *local_kbc_entries, int *local_kbc_counts)
{
	...
	if (i < N) {
		...
		backref_entry.meta = block_entry.meta;
		backref_entry.y = block_entry.y;
		backref_entry.blockposref = i;
		uint64_t calc_y = (uint64_t) block_entry.y + batch_bucket_add_Y;
		uint32_t kbc_id = calc_y / kBC;
		uint32_t KBC_END_ID = KBC_START_ID + KBC_LOCAL_NUM_BUCKETS;
		if ((kbc_id < KBC_START_ID) || (kbc_id > KBC_END_ID)) {
...
void transferBlocksFromHostToDevice(const uint16_t table, const uint32_t batch_id,
	...
	for (uint32_t block_id = 0; block_id < BATCHES; block_id++) {
		...
		if (table == 2) {
			...
			// TODO SAME AS TABLE 2 BUT WITH BLOCK POS REF.
		}
		CUDA_CHECK_RETURN(cudaDeviceSynchronize());
...
void transferBucketedBlocksFromDeviceToHost(const uint16_t table, const uint32_t batch_id,
		char *device_buffer, const size_t DEVICE_ENTRY_SIZE, const int* block_counts) {
	...
	for (uint32_t block_id = 0; block_id < BATCHES; block_id++) {
		...
		if (table == 1) {
			// to block files 21674 ms transfer time. Easiest to manage anyway!
			// to batch files 23041 ms transfer time
			// to table files 27000 ms transfer time.
			// writeT2HostMemToTableFiles(table, batch_id, block_id, &host_criss_cross_blocks[host_bytes_start], bytes_to_copy);
		}
		...
		if (table == 2) {
			...
			// tables was 56..yeesh
			//writeT2HostMemToBlockFiles(table, batch_id, block_id, &host_criss_cross_blocks[host_bytes_start], bytes_to_copy);
		}
	}
	//fclose(pFile);
...
void doT1Batch(uint32_t batch_id, int* local_kbc_num_entries, const uint32_t KBC_START, const uint32_t KBC_END) {
	...
	gpu_chacha8_get_k32_keystream_into_local_kbc_entries<<<numBlocks, blockSize>>>(UINT_MAX, chacha_input,
	...
	start = std::chrono::high_resolution_clock::now();
	transferBucketedBlocksFromDeviceToHost(1, batch_id, device_buffer_B, sizeof(Tx_Bucketed_Meta2), device_block_entry_counts);
	finish = std::chrono::high_resolution_clock::now();
...
void doTxBatch(uint16_t table, uint32_t batch_id) {
	...
	if (table == 2) {
		...
		Tx_Bucketed_Meta4 *bucketed_out = (Tx_Bucketed_Meta4 *) device_buffer_B;
		gpu_find_tx_matches<Tx_Bucketed_Meta4,Tx_Bucketed_Meta4><<<(KBC_END - KBC_START), THREADS_FOR_MATCHING>>>(table, batch_id, KBC_START, KBC_END,
				bucketed_kbc_entries_in, device_local_kbc_num_entries,
				bucketed_out, device_block_entry_counts);
	} else if (table == 4) {
		Tx_Bucketed_Meta4 *bucketed_kbc_entries_in = (Tx_Bucketed_Meta4 *) device_buffer_A;
		Tx_Bucketed_Meta3 *bucketed_out = (Tx_Bucketed_Meta3 *) device_buffer_B;
		gpu_find_tx_matches<Tx_Bucketed_Meta4,Tx_Bucketed_Meta3><<<(KBC_END - KBC_START), THREADS_FOR_MATCHING>>>(table, batch_id, KBC_START, KBC_END,
				bucketed_kbc_entries_in, device_local_kbc_num_entries,
				bucketed_out, device_block_entry_counts);
	} else if (table == 5) {
		Tx_Bucketed_Meta3 *bucketed_kbc_entries_in = (Tx_Bucketed_Meta3 *) device_buffer_A;
		Tx_Bucketed_Meta2 *bucketed_out = (Tx_Bucketed_Meta2 *) device_buffer_B;
		gpu_find_tx_matches<Tx_Bucketed_Meta3,Tx_Bucketed_Meta2><<<(KBC_END - KBC_START), THREADS_FOR_MATCHING>>>(table, batch_id, KBC_START, KBC_END,
				bucketed_kbc_entries_in, device_local_kbc_num_entries,
				bucketed_out, device_block_entry_counts);
	} else if (table == 6) {
		Tx_Bucketed_Meta2 *bucketed_kbc_entries_in = (Tx_Bucketed_Meta2 *) device_buffer_A;
		Tx_Bucketed_Meta2 *NOT_USED = (Tx_Bucketed_Meta2 *) device_buffer_B;
		gpu_find_tx_matches<Tx_Bucketed_Meta2,Tx_Bucketed_Meta2><<<(KBC_END - KBC_START), THREADS_FOR_MATCHING>>>(table, batch_id, KBC_START, KBC_END,
			bucketed_kbc_entries_in, device_local_kbc_num_entries,
			NOT_USED, device_block_entry_counts);
	}
	CUDA_CHECK_RETURN(cudaDeviceSynchronize());
	finish = std::chrono::high_resolution_clock::now();
	...
	if (table < 6) {
		std::cout << "     transferBucketedBlocksFromDeviceToHost\n";
		start = std::chrono::high_resolution_clock::now();
		transferBucketedBlocksFromDeviceToHost(table, batch_id, device_buffer_B, transfer_out_size, device_block_entry_counts);
		CUDA_CHECK_RETURN(cudaDeviceSynchronize());
		finish = std::chrono::high_resolution_clock::now();
		table_transfer_time_ms += std::chrono::duration_cast<milli>(finish - start).count();
		std::cout << "   done. " << std::chrono::duration_cast<milli>(finish - start).count() << " ms\n";
...
void doTx(uint16_t table) {
	std::cout << "do " << table <<"   BATCHES:" << BATCHES << std::endl;
...
void setupMemory() {
	//Tx_Pairing_Chunk_Meta4 *host_criss_cross_blocks;
	//Tx_Pairing_Chunk_Meta4 *device_buffer_A;
	//Tx_Pairing_Chunk_Meta4 *device_buffer_B;
	//int device_kbc_counts; // [kBC_NUM_BUCKETS];
	//uint32_t device_kbc_start_addresses; // [kBC_NUM_BUCKETS];
	//int *pairing_chunks_count;
	//uint32_t host_block_kbc_counts[BATCHES * BATCHES];
